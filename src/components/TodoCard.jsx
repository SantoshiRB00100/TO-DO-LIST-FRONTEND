  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow:  `0 0 0 1px ${pCfg.color}22, 0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.05)`,
        transition: "all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)",
      }}
      onMouseEnter={e => {
        e.currentTarget.style.transform = "perspective(800px) rotateX(-2deg) translateY(-4px)";
        e.currentTarget.style.boxShadow = `0 0 0 1px ${pCfg.color}55, 0 20px 60px rgba(0,0,0,0.5), 0 0 40px ${pCfg.glow}, inset 0 1px 0 rgba(255,255,255,0.08)`;
      }}
      onMouseLeave={e => {
        e.currentTarget.style.transform = "perspective(800px) rotateX(0deg) translateY(0)";
        e.currentTarget.style.boxShadow = `0 0 0 1px ${pCfg.color}22, 0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.05)`;
      }}
    ...
    >
      ...
      <div
        ...
        style={{ background: pCfg.color, boxShadow: `0 0 12px ${pCfg.color}` }}
      />
      ...
      <div className="p-5 pl-6">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span
                ...
                style={{ color: pCfg.color, background: `${pCfg.color}22` }}
              >
          ...
          <button
            ...
            className="opacity-100 md:opacity-0 md:group-hover:opacity-100 text-red-400 hover:text-red-300 active:scale-95 transition-all duration-200 p-2 md:p-1 rounded-lg flex-shrink-0"
        ...
        <div className="mt-3 space-y-1">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs font-black" style={{ color: pCfg.color }}>{fulfillment}%</span>
          </div>
        ...
        >
          {todo.description && (
            <p className="text-sm text-slate-400 mb-3 leading-relaxed">{todo.description}</p>
          )}
          {todo.when && (
          ...
          <div onClick={e => e.stopPropagation()} className="space-y-1">
            <span className="text-xs text-slate-500">Drag to update progress:</span>
            <input
              type="range" min="0" max="100" value={fulfillment}
              onChange={e => handleFulfillmentChange(parseInt(e.target.value))}
